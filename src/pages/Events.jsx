...
import {EventBlock, Loader} from "../components/";

import {fetchEventsLimitFirst, fetchEventsLimit} from "../redux/actions/events";
...
const Events = () => {
    ...
    React.useEffect(() => {
        ...
        if (!allItems.length) {
            dispatch(fetchEventsLimitFirst());
        }
    }, []);

    const onClickFetchEvents = () => {
        dispatch(fetchEventsLimit(page));
    };

    return (
        <>
            <Helmet>
                <title>Мероприятия - Гостиный двор Москва</title>
            </Helmet>
            {isLoaded ? (
                <section className="event">
                    <div className="container">
                        <div className="event-wrapper">
                            <h2 className="title__mb event__title">
                                Мероприятия
                            </h2>
                            <div className="event-block-wrapper">
                                {allItems.map((block, index) => (
                                    <EventBlock
                                        {...block}
